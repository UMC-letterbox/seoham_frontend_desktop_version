...
function TagCreater() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newValue.length > 0) {
      const chosenList = taglist.filter((tl) => tl.includes(newValue));
  ...
  const handleValid = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    !isDubble &&
      setTags((oldTags) => [{ text: tag, id: Date.now() }, ...oldTags]);
    setInputValue("");
    setIsOpenBox(false);
  };

  const handleRemove = (e: React.MouseEvent<HTMLButtonElement>) => {
    const { currentTarget: targetbutton } = e;
